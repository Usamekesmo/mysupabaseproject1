// ====================================================================
// ==      وحدة واجهة المستخدم (ui.js) - النسخة النهائية والمتكاملة
// ====================================================================
...
export function disableQuestionInteraction() {
    if (questionArea) {
        questionArea.querySelectorAll('button, .choice-box, .number-box, .option-div, .audio-choice-btn').forEach(el => {
...
export function displayErrorReview(errorLog) {
    ...
    errorListDiv.innerHTML = errorLog.map(error => `
        <div class="error-review-item">
            <h4>السؤال الذي أخطأت فيه (ID: ${error.question_id || 'N/A'}):</h4>
    ...
    errorListDiv.querySelectorAll('audio, button, .choice-box, .option-div, .audio-choice-btn').forEach(el => {
    ...
    if (lastWeekWinners && lastWeekWinners.length > 0) {
        winnersHtml = `
            <div class="last-week-winners">
                ...
                ${lastWeekWinners.map(winner => `<p>المركز ${winner.rank}: <strong>${winner.clan_name}</strong></p>`).join('')}
            </div>
        `;
    }
...
export function showClansLeaderboardModal(leaderboardData) {
    let content = `<h3>لوحة صدارة القبائل الأسبوعية</h3><div class="leaderboard-list">`;
...
export function renderAvailableChallenges(challenges) {
    const container = document.getElementById('challenges-list-container');
    if (!container) return;

    if (!challenges || challenges.length === 0) {
        container.innerHTML = '<p>لا توجد تحديات متاحة حاليًا. كن أول من ينشئ واحدًا!</p>';
        return;
    }

    container.innerHTML = challenges.map(challenge => {
        const privacyText = challenge.privacy_level === 'public' ? 'عام' : 'للقبيلة فقط';
        const winConditionText = challenge.win_condition === 'highest_score' ? 'أعلى نتيجة' : 'أسرع وقت (بنتيجة كاملة)';
        
        return `
            <div class="challenge-card" data-challenge-id="${challenge.id}">
                <div class="challenge-card-header">
                    <h4>${challenge.title}</h4>
                    <span class="challenge-privacy ${challenge.privacy_level}">${privacyText}</span>
                </div>
                <div class="challenge-card-body">
                    <p><strong>المنشئ:</strong> ${challenge.creator.username}</p>
                    <p><strong>شرط الفوز:</strong> ${winConditionText}</p>
                    <p><strong>المشاركون:</strong> ${challenge.participants_count}</p>
                </div>
                <div class="challenge-card-footer">
                    <button class="start-challenge-button button-secondary" data-challenge-id="${challenge.id}">شارك في التحدي</button>
                </div>
            </div>
        `;
    }).join('');
...
export function showChallengeDetailsModal(challengeDetails) {
    if (!challengeDetails || !challengeDetails.challenge_participations) {
        return showToast("لا يمكن عرض تفاصيل هذا التحدي.", "error");
    }

    const sortedParticipants = challengeDetails.challenge_participations.sort((a, b) => b.score - a.score);

    let participantsHtml = sortedParticipants.map((p, index) => `
        <div class="leaderboard-item">
            <span class="leaderboard-rank">${index + 1}</span>
            <span class="leaderboard-name">${p.players.username}</span>
            <span class="leaderboard-xp">${p.score} نقطة</span>
        </div>
    `).join('');

    if (sortedParticipants.length === 0) {
        participantsHtml = '<p>لم يشارك أحد في هذا التحدي بعد.</p>';
    }

    const content = `
        <h3>تفاصيل تحدي: ${challengeDetails.title}</h3>
        <p>${challengeDetails.description || 'لا يوجد وصف لهذا التحدي.'}</p>
        <hr>
        <h4>لوحة الصدارة الحالية</h4>
